...
#include <stdlib.h>
#include <string.h>
char ch[10][130]={"Anand","Purple","Shivaji","Amruta","MSRTC","SRS"};
char name[32][100]={'\0'};
char number[32][2]={'\0'};
int num1[32]={0};
int trno;
...
void bus()
{
    ...
    printf("\t\t\t\t\t[1]  =>  %s\n",ch[0]); //defined in the beginning of the program
    printf("\n");
    printf("\t\t\t\t\t[2]  =>  %s\n",ch[1]);
    printf("\n");
	printf("\t\t\t\t\t[3]  =>  %s\n",ch[2]);
...
for(i=0; i<booking; i++)//for entering the person name and seat number in the file
{
    ...
    printf("\n======================================================================================================\n\n");
    printf("\n");
      itoa(number, tempstr, 10); //interger to ascii is itoa
...
a = fopen(tempstr2,"a+");//for open the file to write the name in the file
   while(!feof(a)) // feof function is used to determine if the end of the file
   {
      number[i][j] = fgetc(a);
    ...
    do
{
	...
	while(i<10)
	{
	    pword[i]=getch(); //characters are stored to function pword, ascii value is returned by the getch
	    c=pword[i];
	    if(c==13) break; // 13 is ascii value of enter key
	    else printf("*");
	    i++;
	}
	pword[i]='\0'; //after enter string ends thats why we write '\0'
	//char code=pword;
	i=0;
	//scanf("%s",&pword);
